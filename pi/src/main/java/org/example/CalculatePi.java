...
public class CalculatePi
{
    ...
    private static void doPi1() {
        ...
        int max1 = 100000002; //one billion gives you the first 8 digits correct of pi
        int odd = 3;
        double sum = 1;
        boolean sign = false;

       for(int i = 3; i < max1; i+=2){ //1415926735902504 = found after max1 = 100000002
    ...
    public static void checkCorrectDigitCount(String actual, String mine){
        ...
        for(int i = 0; i < mine.length(); i++){
            if(mine.charAt(i) != actual.charAt(i)){
                finalDigitPlace = i - 2;
                System.out.println(finalDigitPlace + " Correct digits! " + i);
                break;
